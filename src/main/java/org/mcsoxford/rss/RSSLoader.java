...
/**
 * Asynchronous loader for RSS feeds. RSS feeds can be loaded in FIFO order or based on priority. Objects of this type
 * can be constructed with one of the provided static methods:
 * <ul>
 * <li>{@link #fifo()}</li>
 * <li>{@link #fifo(int)}</li>
 * <li>{@link #priority()}</li>
 ...
 * </ul>
 * 
 * Completed RSS feed loads can be retrieved with {@link RSSLoader#take()}, {@link RSSLoader#poll()} or
 * {@link RSSLoader#poll(long, TimeUnit)}.
 * 
 * <p>
 * <b>Usage Example</b>
 * 
 * Suppose you want to load an array of RSS feed URIs concurrently before retrieving the results one at a time. You
 * could write this as:
 * 
 * <pre>
 * {@code 
...
public class RSSLoader {
    ...
    /**
     * Instantiate an object which can load RSS feeds asynchronously. The provided {@link BlockingQueue} implementation
     * determines the load behaviour.
     * 
    ...
    /**
     * Returns {@code true} if RSS feeds are currently being loaded, {@code false} otherwise.
     */
    ...
    /**
     * Stop thread after finishing loading pending RSS feed URIs. If this loader has been constructed with
     * {@link #priority()} or {@link #priority(int)}, only RSS feed loads with priority strictly greater than seven (7)
     * are going to be completed.
     * <p>
     * Subsequent invocations of {@link #load(String)} and {@link #load(String, int)} return {@code null}.
     */
    public void stop() {
    ...
    /**
     * Loads the specified RSS feed URI asynchronously. If this loader has been constructed with {@link #priority()} or
     * {@link #priority(int)}, then a default priority of three (3) is used. Otherwise, RSS feeds are loaded in FIFO
     * order.
     * <p>
     * Returns {@code null} if the RSS feed URI cannot be scheduled for loading due to resource constraints or if
     * {@link #stop()} has been previously called.
     * <p>
     * Completed RSS feed loads can be retrieved by calling {@link #take()}. Alternatively, non-blocking polling is
     * possible with {@link #poll()}.
     * 
    ...
    /**
     * Loads the specified RSS feed URI asynchronously. For the specified priority to determine the relative loading
     * order of RSS feeds, this loader must have been constructed with {@link #priority()} or {@link #priority(int)}.
    ...
    /**
     * Retrieves and removes the next Future representing the result of loading an RSS feed, waiting if none are yet
     * present.
     * 
    ...
    /**
     * Retrieves and removes the next Future representing the result of loading an RSS feed or {@code null} if none are
     * present.
     * 
     * @return the {@link Future} representing the loaded RSS feed, or {@code null} if none are present
     * 
     * @throws InterruptedException if interrupted while waiting
     */
    ...
    /**
     * Retrieves and removes the Future representing the result of loading an RSS feed, waiting if necessary up to the
     * specified wait time if none are yet present.
     * 
     * @param timeout how long to wait before giving up, in units of {@code unit}
     * @param unit a {@link TimeUnit} determining how to interpret the {@code timeout} parameter
     * @return the {@link Future} representing the loaded RSS feed, or {@code null} if none are present within the
    ...
    class Loader implements Runnable {
        ...
        /**
         * Keep on loading RSS feeds by dequeuing incoming tasks until the sentinel is encountered.
         */
        @Override
        public void run() {
            try {
    ...
    static class RSSFuture implements Future<RSSFeed>, Comparable<RSSFuture> {
        ...
        @Override
        public synchronized RSSFeed get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException,
                TimeoutException {
