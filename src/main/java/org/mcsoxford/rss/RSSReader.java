...
/**
 * HTTP client to retrieve and parse RSS 2.0 feeds. Callers must call {@link RSSReader#close()} to release all
 * resources.
 * 
 * @author Mr Horn
 */
public class RSSReader implements java.io.Closeable {
    ...
    /**
     * Instantiate a thread-safe HTTP client to retrieve RSS feeds. The injected {@link HttpClient} implementation must
     * be thread-safe. Internal memory consumption and load performance can be tweaked with {@link RSSConfig}.
     * 
     * @param httpclient thread-safe HTTP client implementation
     ...
     * @throws RSSReaderException if RSS feed could not be retrieved because of HTTP error
    ...
    public RSSFeed load(String uri) throws RSSReaderException {
        ...
        try {
            ...
            IoUtils.closeQuietly(feedStream);
        }
    }
